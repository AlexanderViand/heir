...
Lower the `polynomial` dialect to LLVM. This is useful when translating analyses
or transformations based on polynomials directly to a CPU implementation.

### `--heir-basic-mlir-to-llvm`

Lower a subset of standard MLIR dialects to LLVM without relying on any
FHE-specific passes.

### `--mlir-to-secret-arithmetic`

Convert standard MLIR dialects to the `secret` dialect using integer arithmetic.
This forms the basis for lowering to specific FHE schemes.

### `--mlir-to-plaintext-backend`

Translate an MLIR program to a plaintext backend that executes the operations
using the secret dialect. Primarily used for debugging.

### `--mlir-to-bgv`

Lower standard MLIR dialects to the BGV fully homomorphic encryption scheme.

### `--mlir-to-bfv`

Lower standard MLIR dialects to the BFV fully homomorphic encryption scheme.

### `--mlir-to-ckks`

Lower standard MLIR dialects to the CKKS fully homomorphic encryption scheme.

### `--scheme-to-openfhe`

Emit OpenFHE C++ code from scheme-level IR.

### `--scheme-to-lattigo`

Emit Lattigo Go code from scheme-level IR.

### `--scheme-to-tfhe-rs`

Emit code targeting the `tfhe-rs` library from scheme-level IR.

### `--scheme-to-fpt`

Emit code targeting the FPT boolean backend from scheme-level IR.

### `--scheme-to-jaxite`

Emit code targeting the Jaxite backend from scheme-level IR.

### `--convert-to-data-oblivious`

Transform a native program into a data-oblivious equivalent using control flow
and memory rewriting passes.
...
Translate the `openfhe` dialect to C++ code using the OpenFHE PKE API.

### `--emit-openfhe-pke-header`

Emit a header corresponding to the C++ file generated by `--emit-openfhe-pke`.

### `--emit-openfhe-pke-pybind`

Generate a C++ file with `pybind11` bindings for the input `openfhe` dialect IR.

### `--emit-lattigo`

Translate the `lattigo` dialect to Go code using the Lattigo API.

### `--emit-tfhe-rust-hl`

Translate the `tfhe-rs` dialect to a higher level Rust API.

### `--emit-tfhe-rust-bool`

Translate the boolean `tfhe-rs` dialect to Rust code.

### `--emit-tfhe-rust-bool-packed`

Translate the boolean `tfhe-rs` dialect to Rust code targeting the packed FPGA
API.

### `--emit-jaxite`

Translate the Jaxite dialect to Python code for the Jaxite backend.

### `--emit-jaxiteword`

Translate the JaxiteWord dialect to Python code.
