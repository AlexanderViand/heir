...
struct EntryFunctions {
  ...
  func::FuncOp contract;
  func::FuncOp setup;
  func::FuncOp keygen;
  SmallVector<std::pair<unsigned, func::FuncOp>> inputHelpers;
  func::FuncOp preprocess;
  func::FuncOp evaluate;
...
FailureOr<EntryFunctions> findEntryFunctions(ModuleOp module,
  ...
  SmallVector<func::FuncOp> setupFunctions;
  module.walk([&](func::FuncOp function) {
    if (function->hasAttr(kClientSetupFuncAttrName))
      setupFunctions.push_back(function);
  });
  if (setupFunctions.empty())
    return module.emitError("missing a function with client.setup_func");

  func::FuncOp setup;
  if (!requestedEntry.empty()) {
    for (func::FuncOp candidate : setupFunctions) {
      if (getRoleEntry(candidate, kClientSetupFuncAttrName) == requestedEntry) {
        setup = candidate;
        break;
      }
    }
    if (!setup)
      return module.emitError()
             << "missing setup function for entry @" << requestedEntry;
  } else {
    if (setupFunctions.size() != 1)
      return module.emitError(
          "multiple entry interfaces require the entry-function option");
    setup = setupFunctions.front();
  }

  std::optional<StringRef> entry =
      getRoleEntry(setup, kClientSetupFuncAttrName);
  if (!entry)
    return setup.emitOpError("client.setup_func is missing func_name");
  ...
  module.walk([&](func::FuncOp function) {
    ...
    if (matches(kEntryFuncAttrName)) functions.contract = function;
    ...
    for (StringRef role : {kClientEncFuncAttrName, kClientPackFuncAttrName}) {
      ...
      functions.inputHelpers.emplace_back(*index, function);
    }
    if (matches(kClientDecFuncAttrName)) {
      FailureOr<unsigned> index =
          getRoleIndex(function, kClientDecFuncAttrName);
      if (failed(index)) {
        collectionResult = failure();
      } else {
        functions.outputHelpers.emplace_back(*index, function);
      }
    }
  });

  if (failed(collectionResult)) return failure();
  llvm::sort(functions.inputHelpers, [](const auto& lhs, const auto& rhs) {
    return lhs.first < rhs.first;
  });
  llvm::sort(functions.outputHelpers, [](const auto& lhs, const auto& rhs) {
    return lhs.first < rhs.first;
  });
  if (!functions.contract || !functions.keygen || !functions.evaluate)
    return module.emitError() << "entry @" << functions.entryName
                              << " is missing one or more interface roles";
  return functions;
}
...
void addPublicDeclarations(OpBuilder& builder, Location loc,
  ...
  SmallVector<Type> encryptInputs{contextRef, secretKey};
  for (unsigned i = 0; i < logicalInputs.size(); ++i)
    encryptInputs.push_back(
        OpaqueType::get(ctx, "Input" + std::to_string(i) + "&"));
  createEmitCFunction(builder, loc, "Encrypt", encryptInputs, {encryptedInputs},
                      true);
  createEmitCFunction(
...
LogicalResult addEncryptDefinition(OpBuilder& builder, Location loc,
  ...
  for (unsigned i = 0; i < logicalInputs.size(); ++i)
    inputs.push_back(OpaqueType::get(ctx, "Input" + std::to_string(i) + "&"));
  auto function =
  ...
  for (unsigned input = 0; input < logicalInputs.size(); ++input) {
    ...
    for (auto [argumentIndex, type] :
      ...
      arguments.push_back(
          getInputData(builder, loc, function.getArgument(input + 2), type));
      ++dataArguments;
    }
...
LogicalResult buildInterface(ModuleOp module, EntryFunctions& functions) {
  ...
  for (StringRef include :
       {"UserInterface.h", "core/Context.h", "core/Encode.h",
        "core/Parameter.h", "extension/BootContext.h", "extension/EvalPoly.h",
  ...
  emitVerbatim(builder, loc, "using namespace ::cheddar;");
  emitVerbatim(builder, loc, "using Context = ::cheddar::" + contextName + ";");
  emitVerbatim(builder, loc,
               "using SecretKey = ::cheddar::UserInterface<word>*;");
  emitVerbatim(builder, loc,
               "using PublicKey = ::cheddar::UserInterface<word>*;");
  ...
  addPublicDeclarations(builder, loc, logicalInputs, publicResultType);
  emitVerbatim(builder, loc, "}  // namespace " + namespaceName);
