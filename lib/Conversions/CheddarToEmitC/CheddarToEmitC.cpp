...
class TypeConverterImpl : public TypeConverter {
 public:
  explicit TypeConverterImpl(MLIRContext* ctx) {
    ...
    // lowered by upstream MemRefToEmitC; register its type conversion first so
    // our move-only-specific conversion below (added later, hence tried first)
    // can defer to it via std::nullopt.
    mlir::populateMemRefToEmitCTypeConversion(*this);
...
struct ConvertSubViewToPointer
  ...
  LogicalResult matchAndRewrite(
    ...
    rewriter.replaceOpWithNewOp<emitc::ApplyOp>(
        op, emitc::PointerType::get(arrayTy.getElementType()), "&",
        sub.getResult());
    return success();
  }
};
