...
// The cheddar.linear_transform lowering calls this runtime, injected once at
// module scope so generated kernels are self-contained (no consumer prelude
// copy). The code lives in CheddarLintransRuntime.h -- a real, reviewable
// header the compile-only test builds against the CHEDDAR stub -- and the
// genrule in BUILD wraps it into the raw-string .inc included here.
constexpr llvm::StringLiteral kRunLinearTransformShim =
#include "lib/Conversions/CheddarToEmitC/CheddarLintransRuntime.inc"
    ;  // NOLINT
