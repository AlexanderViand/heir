...
struct ConvertCKKSRotateOp : public OpConversionPattern<ckks::RotateOp> {
  ...
  LogicalResult matchAndRewrite(
    ...
    if (staticShift) {
      ...
      if (!inputType)
        return rewriter.notifyMatchFailure(
            op, "cannot determine the CKKS slot count for rotation");
    ...
    if (dynamicShift) {
      ...
      auto result =
          cheddar::HRotOp::create(rewriter, op.getLoc(), resultTy, ctx.value(),
                                  ui.value(), adaptor.getInput(), dest,
...
FailureOr<LinearTransformPlan> getLinearTransformPlan(
    ...
    double ratio, PatternRewriter& rewriter) {
  if (width <= 0)
...
struct ConvertKernelLinearTransformOp
  ...
  LogicalResult matchAndRewrite(
    ...
    auto plan =
        getLinearTransformPlan(op, op.getDiagonals().getType(),
                               op.getDiagonalIndices(), ratio, rewriter);
...
struct ConvertKernelApplyLinearTransformOp
  ...
  LogicalResult matchAndRewrite(
    ...
    double ratio = preparedType.getLogBsgsRatio() == 0
                       ...
                       : std::exp2(preparedType.getLogBsgsRatio());
    auto plan =
        getLinearTransformPlan(op, diagonalWidth.getInt(),
                               diagonalIndices.asArrayRef(), ratio, rewriter);
    if (failed(plan)) return failure();
...
struct LWEToCheddar : public impl::LWEToCheddarBase<LWEToCheddar> {
  void runOnOperation() override {
    ...
    patterns.add<
        ...
        ConvertKernelApplyLinearTransformOp, ConvertKernelEvalChebyshevOp>(
        typeConverter, context);
