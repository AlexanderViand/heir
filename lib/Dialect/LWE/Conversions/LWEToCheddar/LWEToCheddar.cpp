...
static FailureOr<CheddarLevelReduceBucketKind> getCheddarLevelReduceBucketKind(
  ...
  APInt canonicalScale = getNominalPowerOfTwoScaleFromLog2(*logDefaultScale);
  APInt scaledOnce = multiplyUnsignedAPIntExact(canonicalScale, canonicalScale);
  APInt scale = getScalingFactorFromEncodingAttr(
      ctType.getPlaintextSpace().getEncoding());

  if (APInt::isSameValue(scale, canonicalScale)) {
    return CheddarLevelReduceBucketKind::kCanonical;
  }
  if (APInt::isSameValue(scale, scaledOnce)) {
