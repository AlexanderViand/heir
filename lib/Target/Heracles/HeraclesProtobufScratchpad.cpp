...
#include "google/protobuf/util/json_util.h"  // from @com_google_protobuf
#include "heracles/heracles_data_formats.h"  // from @heracles_data_formats
#include "heracles/heracles_proto.h"         // from @heracles_data_formats
...
void fhe_trace_tests() {
  ...
  heracles::fhe_trace::OperandObject *add_args_dest =
      new heracles::fhe_trace::OperandObject();
  ...
  std::cout << "scheme: as-num=" << scheme << " / as-default-string="
            << heracles::common::Scheme_descriptor()
                   ->FindValueByNumber(scheme)
                   ->name()
            << " / as-friendly-string="
            << heracles::common::Scheme_descriptor()
                   ->value(scheme)
  ...
  std::cout << "debug string: " << deserialized_trace.DebugString()
            << std::endl;
}
...
void cpp_data_tests() {
  ...
  auto [new_context, new_testvector] =
      heracles::data::load_data_trace("test.data_trace");
