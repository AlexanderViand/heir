...
LogicalResult OpenFhePkeEmitter::printOperation(MulNoRelinOp op) {
  if (backend_ == OpenfheBackend::FIDESLIB) {
    // FIDESlib has no EvalMultNoRelin — use --auto-relinearize to get
    // openfhe.mul (with implied relin) instead of openfhe.mul_no_relin.
...
LogicalResult OpenFhePkeEmitter::printOperation(ModReduceInPlaceOp op) {
  if (backend_ == OpenfheBackend::FIDESLIB) {
    (void)printEvalInPlaceMethod(op.getResult(), op.getCryptoContext(),
                                 {op.getCiphertext()}, "RescaleInPlace");
    variableNames->mapValueNameToValue(op.getResult(), op.getCiphertext());
    return success();
  }
  (void)printEvalInPlaceMethod(op.getResult(), op.getCryptoContext(),
...
LogicalResult OpenFhePkeEmitter::decodeCore(Location loc, Value input,
  ...
  if (tensorTy) {
    ...
    if (isCKKS && backend_ == OpenfheBackend::FIDESLIB) {
      // FIDESlib returns real-valued packed values directly.
    ...
    if (isCKKS && backend_ != OpenfheBackend::FIDESLIB) {
      // OpenFHE CKKS: drop complex down to real
      ...
      // FIDESlib CKKS (already real) or BGV/BFV: direct copy
...
LogicalResult OpenFhePkeEmitter::printOperation(GenParamsOp op) {
  ...
  if (backend_ == OpenfheBackend::FIDESLIB) {
    ...
    if (evalAddCount != 0 || keySwitchCount != 0 ||
        op.getMaxRelinSkDeg() != 0 || op.getEncryptionTechniqueExtended()) {
      return emitError(op.getLoc(),
    ...
    if (op.getInsecure()) {
      os << paramsName << ".SetSecurityLevel(fideslib::HEStd_NotSet);\n";
    }
