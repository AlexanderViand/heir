  ...
  LogicalResult status =
      llvm::TypeSwitch<Operation&, LogicalResult>(op)
          ...
          .Case<AddInPlaceOp, AddOp, AddPlainInPlaceOp, AddPlainOp, AutomorphOp,
                ...
                ModReduceInPlaceOp, ModReduceOp, MulConstInPlaceOp,
                MulConstOp, MulNoRelinOp, MulOp, MulPlainOp, NegateInPlaceOp,
                NegateOp, RelinInPlaceOp, RelinOp, RotOp, SetupBootstrapOp,
...
bool OpenFhePkeEmitter::hasPrecomputeableOps(func::FuncOp funcOp) {
  ...
  funcOp.walk([&](Operation* op) {
    if (isa<LinearTransformOp, MakeCKKSPackedPlaintextOp>(op)) {
      found = true;
...
SmallVector<Value> OpenFhePkeEmitter::getPrecomputeArguments(
  ...
  for (Value arg : funcOp.getArguments()) {
    if (isa<CryptoContextType>(arg.getType()) || isa<CiphertextType>(arg.getType()) ||
        isa<PlaintextType>(arg.getType()) || isa<PrivateKeyType>(arg.getType()) ||
...
LogicalResult OpenFhePkeEmitter::emitPrecomputeSignature(func::FuncOp funcOp) {
  ...
  for (Value arg : precomputeArgs) {
    ...
    if (isa<ShapedType>(arg.getType())) {
      ...
      argStrings.push_back(typeName + " " + variableNames->getNameForValue(arg));
    }
  }
...
LogicalResult OpenFhePkeEmitter::emitPrecomputeFunction(func::FuncOp funcOp) {
  ...
  funcOp.walk([&](Operation* op) {
    if (auto linearTransform = dyn_cast<LinearTransformOp>(op)) {
      ...
      std::string inputVarName = variableNames->getNameForValue(makePlain.getValue());
      ...
      os << "auto " << filledLengthName << " = " << contextName
         << "->GetCryptoParameters()->GetElementParams()->GetRingDimension() / 2;\n";
      ...
      os << "heir_ckks_plaintext_cache()[heir_cache_key(\"" << cacheKey
         ...
         << "->MakeCKKSPackedPlaintext(" << filledName << ");\n";
    }
    return WalkResult::advance();
  });
...
LogicalResult OpenFhePkeEmitter::printOperation(
  ...
  std::string cc = variableNames->getNameForValue(resultCC.value());
  auto parentFunc = op->getParentOfType<func::FuncOp>();
  bool cacheable = isa<BlockArgument>(op.getValue()) && static_cast<bool>(parentFunc);
  std::string cacheKey;
  if (cacheable) {
    ...
    os << "auto " << cacheVar << " = heir_ckks_plaintext_cache()[heir_cache_key(\""
       << cacheKey << "\", " << cc << ")];\n";
    os << "if (" << cacheVar << ") {\n";
