...
void registerToFideslibPkeTranslation() {
  TranslateFromMLIRRegistration reg(
      "emit-fideslib-pke",
      "translate the openfhe dialect to C++ code against the FIDESlib API",
      [](Operation* op, llvm::raw_ostream& output) {
        return translateToFideslibPke(op, output, options->weightsFile,
                                      options->skipVectorResizing);
      },
      registerRelevantDialects);
}
...
void registerToFideslibPkeHeaderTranslation() {
  TranslateFromMLIRRegistration reg(
      "emit-fideslib-pke-header",
      "Emit a header corresponding to the C++ file generated by "
      "--emit-fideslib-pke",
      [](Operation* op, llvm::raw_ostream& output) {
        return translateToFideslibPkeHeader(
            op, output, options->openfheDebugHelperIncludePath);
      },
      registerRelevantDialects);
}
