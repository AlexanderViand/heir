# HEIR Integration with SimFHE: Technical Documentation for SimFHE Maintainers

## Executive Summary

HEIR (Homomorphic Encryption Intermediate Representation) is an MLIR-based
compiler toolchain for FHE applications. This document describes HEIR's
integration with SimFHE for performance estimation of CKKS-based FHE programs.
The integration allows HEIR to automatically generate SimFHE Python code from
high-level program descriptions, enabling performance analysis without manual
SimFHE code writing.
...
HEIR is a compiler toolchain that:

- Takes high-level programs (C++, Python, MLIR) as input
- Optimizes them for FHE execution
- Generates code for multiple FHE backends (OpenFHE, TFHE-rs, Lattigo, etc.)
- Now includes SimFHE as a target for performance estimation
