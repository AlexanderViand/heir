...
// CHECK: func.func @dead_local(%[[CTX:.*]]: !emitc.ptr<!emitc.opaque<"Context<word>">>, %[[IN:.*]]: !emitc.lvalue<!emitc.opaque<"Ciphertext<word>">>, %[[DST:.*]]: !emitc.lvalue<!emitc.opaque<"Ciphertext<word>">>)
// CHECK: %[[TMP:.*]] = "emitc.variable"
// CHECK: "Neg"(%[[TMP]], %[[IN]])
// CHECK: emitc.verbatim "{} = std::move({});" args %[[DST]], %[[TMP]]
// CHECK-NOT: "Copy"
func.func @dead_local(%ctx: !cheddar.context, %in: memref<!cheddar.ciphertext>, %dst: memref<!cheddar.ciphertext>) {
...
// CHECK-NOT: "Copy"
func.func @dead_local_dealloc(%ctx: !cheddar.context, %in: memref<!cheddar.ciphertext>, %dst: memref<!cheddar.ciphertext>) {
  %tmp = memref.alloc() : memref<!cheddar.ciphertext>
  cheddar.neg %ctx, %in, %tmp : (!cheddar.context, memref<!cheddar.ciphertext>, memref<!cheddar.ciphertext>) -> ()
  memref.copy %tmp, %dst : memref<!cheddar.ciphertext> to memref<!cheddar.ciphertext>
...
// CHECK: "Copy"
// CHECK-NOT: std::move
func.func @live_local(%ctx: !cheddar.context, %in: memref<!cheddar.ciphertext>, %dst: memref<!cheddar.ciphertext>) {
  %tmp = memref.alloc() : memref<!cheddar.ciphertext>
  cheddar.neg %ctx, %in, %tmp : (!cheddar.context, memref<!cheddar.ciphertext>, memref<!cheddar.ciphertext>) -> ()
  memref.copy %tmp, %dst : memref<!cheddar.ciphertext> to memref<!cheddar.ciphertext>
...
// CHECK: "Copy"
// CHECK-NOT: std::move
func.func @outer_temporary(%ctx: !cheddar.context, %in: memref<!cheddar.ciphertext>, %dst: memref<!cheddar.ciphertext>, %n: index) {
...
// CHECK: emitc.verbatim "{} = std::move({});"
// CHECK-NOT: "Copy"
func.func @use_in_region_before(%ctx: !cheddar.context, %in: memref<!cheddar.ciphertext>, %dst: memref<!cheddar.ciphertext>, %c: i1) {
  %tmp = memref.alloc() : memref<!cheddar.ciphertext>
  scf.if %c {
    cheddar.neg %ctx, %in, %tmp : (!cheddar.context, memref<!cheddar.ciphertext>, memref<!cheddar.ciphertext>) -> ()
  }
...
// CHECK: "Copy"
// CHECK-NOT: std::move
func.func @use_in_region_after(%ctx: !cheddar.context, %in: memref<!cheddar.ciphertext>, %dst: memref<!cheddar.ciphertext>, %c: i1) {
  %tmp = memref.alloc() : memref<!cheddar.ciphertext>
  cheddar.neg %ctx, %in, %tmp : (!cheddar.context, memref<!cheddar.ciphertext>, memref<!cheddar.ciphertext>) -> ()
  memref.copy %tmp, %dst : memref<!cheddar.ciphertext> to memref<!cheddar.ciphertext>
  scf.if %c {
...
// CHECK: "Copy"
// CHECK-NOT: std::move
func.func @viewed_temporary(%ctx: !cheddar.context, %in: memref<!cheddar.ciphertext>, %dst: memref<2x!cheddar.ciphertext>) {
...
// A rank-1 temporary moves as a whole std::array.
// CHECK: func.func @dead_local_array
// CHECK: emitc.verbatim "{} = std::move({});"
// CHECK-NOT: "Copy"
func.func @dead_local_array(%ctx: !cheddar.context, %in: memref<2x!cheddar.ciphertext>, %dst: memref<2x!cheddar.ciphertext>) {
  %tmp = memref.alloc() : memref<2x!cheddar.ciphertext>
...
// CHECK-NOT: "Copy"
// CHECK: return
func.func @self_copy_dead_local(%ctx: !cheddar.context, %in: memref<!cheddar.ciphertext>) {
  %tmp = memref.alloc() : memref<!cheddar.ciphertext>
  cheddar.neg %ctx, %in, %tmp : (!cheddar.context, memref<!cheddar.ciphertext>, memref<!cheddar.ciphertext>) -> ()
