...
// CHECK: error: 'orion.chebyshev' op requires Orion implementation style `opaque`, but got `unsupported`
...
module attributes {scheme.ckks, ckks.schemeParam = #ckks.scheme_param<logN = 3, Q = [17], P = [257], logDefaultScale = 4>} {
  func.func @chebyshev(%ct: !ct_L0) -> !ct_L0 {
    %ct_0 = orion.chebyshev %ct {coefficients = [0.0, 0.75, 0.0, 0.25], domain_end = 1.000000e+00 : f64, domain_start = -1.000000e+00 : f64, orion.impl_style = "unsupported"} : (!ct_L0) -> !ct_L0
    return %ct_0 : !ct_L0
  }
}
