// RUN: not heir-opt --annotate-orion="linear-transform-impl-style=diagonal-basic" --lwe-to-openfhe %s 2>&1 | FileCheck %s

// CHECK: error: 'orion.linear_transform' op implementation style `diagonal-basic` must be lowered by `--lower-orion` before targeting OpenFHE
...
module attributes {scheme.ckks, ckks.schemeParam = #ckks.scheme_param<logN = 13, Q = [536903681, 67043329, 66994177, 67239937, 66961409, 66813953], P = [536952833, 536690689], logDefaultScale = 26>} {
  func.func @linear_transform(%cc: !openfhe.crypto_context, %ct: !ct_L5, %arg0: tensor<2x4xf64>) -> !ct_L5 {
    %ct_0 = orion.linear_transform %ct, %arg0 {block_col = 0 : i32, block_row = 0 : i32, bsgs_ratio = 2.000000e+00 : f64, diagonal_count = 2 : i32, diagonal_indices = array<i32: 0, 3>, orion_level = 5 : i32, slots = 8 : i32} : (!ct_L5, tensor<2x4xf64>) -> !ct_L5
    return %ct_0 : !ct_L5
  }
}
