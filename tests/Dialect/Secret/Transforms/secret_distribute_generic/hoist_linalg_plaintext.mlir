...
module attributes {ckks.schemeParam = #ckks.scheme_param<logN = 15, Q = [36028797019488257, 35184372744193, 35184373006337, 35184373989377, 35184376545281, 35184377331713, 35184378511361, 35184378707969, 35184379035649, 35184380870657, 35184381591553], P = [36028797020209153, 36028797020602369, 36028797020864513, 36028797023420417], logDefaultScale = 45>, scheme.ckks} {
  func.func @test_linalg_hoist_plaintext(%arg0: !secret.secret<tensor<1x1024xf32>> {mgmt.mgmt = #mgmt.mgmt<level = 10, scale = 1237940039285380274899124224 : i92>}, %arg1: !secret.secret<tensor<1x1024xf32>> {mgmt.mgmt = #mgmt.mgmt<level = 10, scale = 35184372088832>, tensor_ext.original_type = #tensor_ext.original_type<originalType = tensor<1x1x32x32xf32>, layout = #tensor_ext.layout<"{ [i0, i1, i2, i3] -> [ct, slot] : i0 = 0 and i1 = 0 and ct = 0 and (-32i2 - i3 + slot) mod 1024 = 0 and 0 <= i2 <= 31 and 0 <= i3 <= 31 and 0 <= slot <= 1023 }">>}) -> (!secret.secret<tensor<1x1024xf32>> {mgmt.mgmt = #mgmt.mgmt<level = 10, scale = 1237940039285380274899124224 : i92>, tensor_ext.original_type = #original_type}) {
    ...
    %1 = secret.generic(%arg0: !secret.secret<tensor<1x1024xf32>> {mgmt.mgmt = #mgmt.mgmt<level = 10, scale = 35184372088832>}) {
    ^body(%input0: tensor<1x1024xf32>):
      ...
      %3 = mgmt.init %2 {mgmt.mgmt = #mgmt.mgmt<level = 10, scale = 1237940039285380274899124224 : i92>} : tensor<1x1024xf32>
      %4 = arith.addf %input0, %3 {mgmt.mgmt = #mgmt.mgmt<level = 10, scale = 1237940039285380274899124224 : i92>} : tensor<1x1024xf32>
      secret.yield %4 : tensor<1x1024xf32>
    } -> (!secret.secret<tensor<1x1024xf32>> {mgmt.mgmt = #mgmt.mgmt<level = 10, scale = 1237940039285380274899124224 : i92>})
