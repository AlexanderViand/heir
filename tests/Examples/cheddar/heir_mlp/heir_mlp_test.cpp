...
void mnist__preprocessing(const cheddar::Encoder<word>& encoder,
                          float* first_weight, float* second_weight,
                          float* second_bias, float* first_bias,
                          cheddar::Context<word>* ctx,
                          std::shared_ptr<LinearTransform>& first_transform,
                          std::shared_ptr<LinearTransform>& second_transform,
...
void mnist__preprocessed(
    ...
    const std::array<Ct, 1>& input, const std::array<Pt, 3>& plaintexts,
    const std::shared_ptr<LinearTransform>& first_transform,
    const std::shared_ptr<LinearTransform>& second_transform,
    std::array<Ct, 1>& out);
...
TEST(CheddarHeirMlpE2E, MatchesDegreeFivePlaintextCircuit) {
  ...
  mnist__preprocessing(ctx->encoder_, first_weight.data(), second_weight.data(),
                       second_bias.data(), first_bias.data(), ctx.get(),
                       first_transform, second_transform, plaintexts);
  std::cerr << "HeirMLP preprocessing took "
            << std::chrono::duration<double>(std::chrono::steady_clock::now() -
                                             preprocessing_start)
                   .count()
            << " seconds\n";
  ASSERT_NE(first_transform, nullptr);
  ASSERT_NE(second_transform, nullptr);
  ...
  mnist__preprocessed(ctx.get(), ctx->encoder_, ui.get(), evk, evk_map,
                      ...
                      plaintexts, first_transform, second_transform, evaluated);
  ...
  mnist__preprocessed(
      ctx.get(), ctx->encoder_, ui.get(), evk, evk_map, first_weight.data(),
      first_bias.data(), second_weight.data(), second_bias.data(), encrypted,
      plaintexts, first_transform, second_transform, evaluated_again);
