...
void lola__preprocessed(
    ...
    const std::array<Pt, 21>& plaintexts,
...
TEST(CheddarLoLaE2E, MatchesPlaintextMnistLogits) {
  ...
  lola__preprocessing(ctx->encoder_, ctx.get(), first_transform,
                      second_transform, plaintexts);
  ...
  lola__preprocessed(ctx.get(), ctx->encoder_, ui.get(), evk, evk_map,
                     encrypted, plaintexts, first_transform, second_transform,
                     evaluated);
