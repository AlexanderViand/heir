...
TEST(CheddarPipelineEvalChebyshevE2E, MatchesPlaintextDegreeFive) {
  ...
  chebyshev_cube__encrypt__arg0(ctx.get(), ctx->encoder_, ui.get(), evk, input,
                                ui.get(), encrypted);
  ...
  float actual[8];
  chebyshev_cube__decrypt__result0(ctx.get(), ctx->encoder_, ui.get(), evk,
                                   evaluated, ui.get(), actual);
  for (int i = 0; i < 8; ++i) {
